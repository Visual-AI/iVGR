...
class FSDPWorker(Worker):
    ...
    def init_model(self):
        ...
        if self._has_ref:
            # Use ref's own model config if provided; otherwise fall back to the actor's model config
            ref_model_config = self.config.ref.model if self.config.ref.model is not None else self.config.actor.model
            self._build_model_optimizer(
