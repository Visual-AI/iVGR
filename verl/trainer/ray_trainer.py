...
def compute_advantage(data: DataProto, adv_estimator: AdvantageEstimator, gamma: float = 1.0, lam: float = 1.0):
    ...
    # Add support for SEGMENT_GRPO
    if adv_estimator == AdvantageEstimator.SEGMENT_GRPO:
        if "segment1_rewards" in data.batch:
            adv_inputs["segment1_rewards"] = data.batch["segment1_rewards"]
        if "segment2_rewards" in data.batch:
            adv_inputs["segment2_rewards"] = data.batch["segment2_rewards"]
        if "draft_end_positions" in data.batch:
            adv_inputs["draft_end_positions"] = data.batch["draft_end_positions"]
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        if (
            config.algorithm.adv_estimator in (AdvantageEstimator.GRPO, AdvantageEstimator.RLOO, AdvantageEstimator.SEGMENT_GRPO)
            and config.worker.rollout.n == 1
        ):
            raise ValueError("GRPO, RLOO, and SEGMENT_GRPO algorithms need `config.worker.rollout.n > 1`.")

        if config.trainer.max_steps is not None:
            # max_steps is treated as the number of additional steps to train
            # Final training_steps = initial_global_step + max_steps
            self.training_steps = config.trainer.initial_global_step + config.trainer.max_steps
            ...
            self.training_steps += config.trainer.initial_global_step
        else:
            self.training_steps = len(train_dataloader) * config.trainer.total_epochs
            self.training_steps += config.trainer.initial_global_step

        # For optimizer, we use the number of training steps from the current stage
        actual_training_steps = self.training_steps - config.trainer.initial_global_step
        config.worker.actor.optim.training_steps = actual_training_steps
        config.worker.critic.optim.training_steps = actual_training_steps
        print(f"Total training steps: {self.training_steps} (starting from step {config.trainer.initial_global_step}, training {actual_training_steps} steps)")
    ...
    def _save_checkpoint(self) -> None:
        ...
        # Save reference_cache (if the reward function supports it)
        try:
            ...
            if reward_function_path and os.path.exists(reward_function_path):
                ...
                if hasattr(reward_module, "save_reference_cache"):
                    cache_path = os.path.join(folder_path, "reference_cache.json")
                    reward_module.save_reference_cache(cache_path)
        except Exception as e:
            print(f"[WARNING] Failed to save reference cache: {e}")
    ...
    def _load_checkpoint(self) -> None:
        ...
        # Load reference_cache (if the reward function supports it)
        try:
            import importlib.util
            reward_function_path = self.config.worker.reward.reward_function
            if reward_function_path and os.path.exists(reward_function_path):
                ...
                reward_module = importlib.util.module_from_spec(spec)
                spec.loader.exec_module(reward_module)
                if hasattr(reward_module, "load_reference_cache"):
                    cache_path = os.path.join(load_checkpoint_path, "reference_cache.json")
                    reward_module.load_reference_cache(cache_path)
        except Exception as e:
            print(f"[WARNING] Failed to load reference cache: {e}")

    def _load_reference_cache_only(self) -> None:
        """
        Load reference cache from a separate path without loading model checkpoint.
        This is useful for multi-stage training where we want to continue GRPO
        after SFT with a fresh model but keep the reference cache from previous GRPO stage.
        """
        if self.config.trainer.load_reference_cache_path is None:
            return
        
        load_reference_cache_path = self.config.trainer.load_reference_cache_path
        
        try:
            import importlib.util
            reward_function_path = self.config.worker.reward.reward_function
            if reward_function_path:
                # Handle path with function name (e.g., "path/to/file.py:function_name")
                if ":" in reward_function_path:
                    reward_function_path = reward_function_path.split(":")[0]
                
                if os.path.exists(reward_function_path):
                    spec = importlib.util.spec_from_file_location("reward_module", reward_function_path)
                    reward_module = importlib.util.module_from_spec(spec)
                    spec.loader.exec_module(reward_module)
                    if hasattr(reward_module, "load_reference_cache"):
                        reward_module.load_reference_cache(load_reference_cache_path)
                        print(f"[Reference Cache] Loaded from separate path: {load_reference_cache_path}")
        except Exception as e:
            print(f"[WARNING] Failed to load reference cache from {load_reference_cache_path}: {e}")
    ...
    def fit(self):
        ...
        # Initialize global_step with initial_global_step (for multi-stage training)
        self.global_step = self.config.trainer.initial_global_step
        ...
        # Load reference cache from separate path if specified (for multi-stage training)
        self._load_reference_cache_only()
            ...
            with timer("step", timing_raw):
                ...
                with timer("adv", timing_raw):
                    if "token_level_scores" not in batch.batch:
                        ...
                        # Pull segment info into the batch if present
                        if "_segment1_rewards_tensor" in reward_metrics:
                            batch.batch["segment1_rewards"] = reward_metrics.pop("_segment1_rewards_tensor")
                        if "_segment2_rewards_tensor" in reward_metrics:
                            batch.batch["segment2_rewards"] = reward_metrics.pop("_segment2_rewards_tensor")
                        if "_draft_end_positions_tensor" in reward_metrics:
                            batch.batch["draft_end_positions"] = reward_metrics.pop("_draft_end_positions_tensor")

                        # Process regular metrics
                        processed_metrics = {}
                        for k, v in reward_metrics.items():
                            if not k.startswith("_"):  # Process non-special keys only
                                processed_metrics[f"reward/{k}"] = v
                        metrics.update(reduce_metrics(processed_metrics))
