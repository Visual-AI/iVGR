...
@register_adv_estimator(AdvantageEstimator.SEGMENT_GRPO)
def compute_segment_grpo_advantage(
    token_level_rewards: torch.Tensor, 
    response_mask: torch.Tensor, 
    index: torch.Tensor, 
    segment1_rewards: torch.Tensor = None,
    segment2_rewards: torch.Tensor = None,
    draft_end_positions: torch.Tensor = None,
    eps: float = 1e-6, 
    **kwargs
) -> tuple[torch.Tensor, torch.Tensor]:
    """
    Compute advantage for Segment GRPO, operating on two segments with separate rewards.
    
    Args:
        token_level_rewards: `(torch.Tensor)`
            shape: (bs, response_length) - legacy rewards for compatibility
        response_mask: `(torch.Tensor)`
            shape: (bs, response_length)
        index: `(torch.Tensor)`
            shape: (bs,) - group index for normalization
        segment1_rewards: `(torch.Tensor)`
            shape: (bs,) - rewards for segment 1 (up to </draft>)
        segment2_rewards: `(torch.Tensor)`
            shape: (bs,) - rewards for segment 2 (after </draft>)
        draft_end_positions: `(torch.Tensor)`
            shape: (bs,) - position where segment 1 ends (</draft> end position)
        eps: `(float)`
            epsilon value to avoid division by zero
    ...
    # Fall back to the original GRPO if no segment info is provided
    if segment1_rewards is None or segment2_rewards is None or draft_end_positions is None:
    ...
    # Run GRPO normalization on segment1 and segment2 separately
    def normalize_segment_rewards(segment_rewards, group_indices):
        """Normalize segment rewards within each group."""
        id2score = defaultdict(list)
        id2mean, id2std = {}, {}

        for i in range(len(segment_rewards)):
            # Handle different group_indices types (tensor or list of strings)
            if hasattr(group_indices[i], 'item'):
                group_id = group_indices[i].item()  # tensor type
            else:
                group_id = group_indices[i]  # string or other types
            id2score[group_id].append(segment_rewards[i])
        ...
        for idx in id2score:
            if len(id2score[idx]) > 1:
                ...
                # With a single sample, mean is itself and std is set to 1 to avoid divide-by-zero
                id2mean[idx] = id2score[idx][0]
                id2std[idx] = torch.tensor(1.0, device=segment_rewards.device)

        normalized_scores = torch.zeros_like(segment_rewards)
        for i in range(len(segment_rewards)):
            # Handle different group_indices types
            if hasattr(group_indices[i], 'item'):
                group_id = group_indices[i].item()  # tensor type
            else:
                group_id = group_indices[i]  # string or other types
            normalized_scores[i] = (segment_rewards[i] - id2mean[group_id]) / (id2std[group_id] + eps)

        return normalized_scores

    # Normalize the two segments separately
    normalized_segment1 = normalize_segment_rewards(segment1_rewards, index)
    normalized_segment2 = normalize_segment_rewards(segment2_rewards, index)

    # Build the token-level reward matrix
    token_level_advantages = torch.zeros_like(token_level_rewards)

    for i in range(bsz):
        draft_end_pos = draft_end_positions[i].item()

        # Handle invalid draft_end_pos (format errors)
        if draft_end_pos < 0 or draft_end_pos > seq_len:
            # Format error: fall back to original logic — first half segment1, second half segment2
            mid_pos = seq_len // 2
            token_level_advantages[i, :mid_pos] = normalized_segment1[i]
            token_level_advantages[i, mid_pos:] = normalized_segment2[i]
        else:
            # Normal case: segment1 up to draft_end_pos, segment2 from draft_end_pos onward
            # Make sure draft_end_pos is within the valid range
            draft_end_pos = max(0, min(draft_end_pos, seq_len))
            if draft_end_pos == 0:
                # draft_end_pos is 0: the whole sequence is segment2
                token_level_advantages[i, :] = normalized_segment2[i]
            elif draft_end_pos == seq_len:
                # draft_end_pos equals seq_len: the whole sequence is segment1
                token_level_advantages[i, :] = normalized_segment1[i]
            else:
                # Normal split
                token_level_advantages[i, :draft_end_pos] = normalized_segment1[i]
                token_level_advantages[i, draft_end_pos:] = normalized_segment2[i]

    # Apply the response mask
    advantages = token_level_advantages * response_mask
    returns = advantages  # In segment GRPO, advantages are the returns
