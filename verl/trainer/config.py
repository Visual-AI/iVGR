...
class DataConfig:
    ...
    paired_ratio: float = 0.7
    """ratio of paired A/B samples in each batch for mixed dual-group training (0.0-1.0)"""
...
class TrainerConfig:
    ...
    """initial global step for logging and checkpoint naming (used when resuming from SFT model)"""
    load_reference_cache_path: Optional[str] = None
    """path to load reference cache separately (without loading model checkpoint)"""
    ...
    def post_init(self):
        ...
        if self.load_reference_cache_path is not None:
            if os.path.exists(self.load_reference_cache_path):
                self.load_reference_cache_path = os.path.abspath(self.load_reference_cache_path)
            else:
                print(f"Reference cache {self.load_reference_cache_path} not found.")
                self.load_reference_cache_path = None
